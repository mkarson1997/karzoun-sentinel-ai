...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="sentinelai", description="Evaluate and regression-test LLM outputs")
    ...
    compare = subparsers.add_parser("compare", help="Compare current report against a baseline")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "evaluate":
        result = EvaluationSuite().evaluate(load_jsonl(args.dataset), workers=max(1, args.workers))
        payload = suite_to_dict(result)
        if args.json_path:
            save_json(result, args.json_path)
        if args.markdown_path:
            Path(args.markdown_path).write_text(render_markdown(result), encoding="utf-8")
        print(json.dumps(payload, indent=2, ensure_ascii=False))
        return 0 if result.passed or args.allow_failures else 1

    baseline = json.loads(Path(args.baseline).read_text(encoding="utf-8"))
    current = json.loads(Path(args.current).read_text(encoding="utf-8"))
    comparison = compare_reports(baseline, current, max_score_drop=args.max_score_drop)
